...
const NewsletterPopup = () => {
  ...
  // Show popup once a month if the user hasn't subscribed
  useEffect(() => {
    // Never show to users who already subscribed
    const subscribed = localStorage.getItem("newsletterSubscribed");
    if (subscribed) return;

    // Don't show if dismissed/shown within the last 30 days
    const expiry = localStorage.getItem("newsletterPopupExpiry");
    if (expiry && new Date() < new Date(expiry)) return;
