...
const BESTSELLER_NAMES = [
  ...
  "grace mini-skirt set",
];

export default function BestsellersClient({ products }) {
  const bestsellers =
    products?.filter((p) => BESTSELLER_NAMES.includes(p.name?.toLowerCase().trim())) || [];

  return (
    <main className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-8 sm:py-10">
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-light text-slate-900 mb-2 font-playfair">Bestsellers</h1>
          <p className="text-sm text-slate-500 max-w-xl mx-auto font-poppins">
            Our most loved and frequently purchased pieces
          </p>
        </div>
